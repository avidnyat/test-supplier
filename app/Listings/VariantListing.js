        ...
        var details = this.props.listing.variants.map(function (item, index) {
            ...
            var subVariants = item.sub_variants.map(function (items) {
                var subVariantsPrice = $.map(items.fixed_pricings, function (val) {
                    ...
                    return (
                        <div>
                            <span className="price"><i className="fa fa-inr" aria-hidden="true"></i>{ val.mrp }</span>
                            { '  ' + inventoryObj[0].name }
                        </div>
                    );
                });
            ...
            if(index != 0){
                return (
                    <div>
                        <hr />
                        <h3>0{ index+1 }. { item.name }
                            <button className="btn btn-line btn-secondary edit-button-btn"
